namespace ErpSystem.Tests.Products
{
    ...
    public class ProductTestsDeleteProduct
    {
        [Fact]
        public void CheckCorrectSearchBySearchByProductNameAndId()
        {
            var optionsBuilder = new DbContextOptionsBuilder<ErpSystemDbContext>().UseInMemoryDatabase("testDeleteProduct");
            ...
            var result = product.DeleteExistingProduct(1, "First");

            Assert.True(result.IsCompleted);

            result = product.DeleteExistingProduct(2, "Second");

            Assert.True(result.IsCompleted);

            result = product.DeleteExistingProduct(1, "First");
