namespace ErpSystem.Tests.Sales
{
    ...
    public class SalesTestsCreateSale
    {
        [Fact]
        public void CheckCorrectCreateSale()
        {
            var optionsBuilder = new DbContextOptionsBuilder<ErpSystemDbContext>().UseInMemoryDatabase("testCreateSale");
            ...
            var firstProductResult = dbContext.Sales.Select(x => x).ToList();

            Assert.True(firstProductResult[0].SingleProudctSalePrice == 72);
            Assert.True(firstProductResult[0].Customer.CompanyName == "Company1");
            Assert.True(warehouseProductsFromDb.Where(x => x.ProductId == firstProduct.Id).Select(y => y.ProductsAvailable).FirstOrDefault() == 8);
            Assert.True(warehouseFromDb.CurrentBoxesFrontSpaceFree == 300);
            ...
            var firstProductResultSecondSale = dbContext.Sales.Select(x => x).ToList();

            Assert.True(firstProductResultSecondSale[1].SingleProudctSalePrice == 72);
            Assert.True(firstProductResultSecondSale[1].Customer.CompanyName == "Company1");
            Assert.True(warehouseProductsFromDb.Where(x => x.ProductId == firstProduct.Id).Select(y => y.ProductsAvailable).FirstOrDefault() == 0);
            Assert.True(warehouseFromDb.CurrentBoxesFrontSpaceFree == 310);
            ...
            var secondProductResult = dbContext.Sales.Select(x => x).ToList();

            Assert.Equal(8.27m, secondProductResult[2].SingleProudctSalePrice, 2);
            Assert.True(secondProductResult[2].Customer.CompanyName == "Company1");
            Assert.True(warehouseProductsFromDb.Where(x => x.ProductId == secondProduct.Id).Select(y => y.ProductsAvailable).FirstOrDefault() == 399);
            Assert.True(warehouseFromDb.CurrentBoxesFrontSpaceFree == 410);
            ...
            var thirddProductResult = dbContext.Sales.Select(x => x).ToList();

            Assert.Equal(9.99m, thirddProductResult[3].SingleProudctSalePrice, 2);
            Assert.True(warehouseProductsFromDb.Where(x => x.ProductId == thirdProduct.Id).Select(y => y.ProductsAvailable).FirstOrDefault() == 301);
            Assert.True(warehouseFromDb.CurrentPalletsSpaceFree == 8);
            ...
            var thirddProductResultScondSale = dbContext.Sales.Select(x => x).ToList();

            Assert.Equal(7.59m, thirddProductResultScondSale[4].SingleProudctSalePrice, 2);
            Assert.True(warehouseProductsFromDb.Where(x => x.ProductId == thirdProduct.Id).Select(y => y.ProductsAvailable).FirstOrDefault() == 299);
            Assert.True(warehouseFromDb.CurrentPalletsSpaceFree == 9);
        }
    }
}