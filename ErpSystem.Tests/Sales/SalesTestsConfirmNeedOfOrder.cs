namespace ErpSystem.Tests.Sales
{
    ...
    {
        [Fact]
        public void CheckCorrectConfirmNeedOfOrder()
        {
            var optionsBuilder = new DbContextOptionsBuilder<ErpSystemDbContext>().UseInMemoryDatabase("testConfirmNeedOfOrder");
            ...
            Assert.True(result.Where(x => x.ConfimBeenNoticed == false).Count() == 3);

            sale.ConfirmNeedOfOrder(result[0]);

            result = dbContext.DeliveryNeededProducts.Select(x => x).ToList();

            Assert.True(result.Where(x => x.ConfimBeenNoticed == false).Count() == 0);
        }
    }
}
