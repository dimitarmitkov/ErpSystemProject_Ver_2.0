...
{
    public class SalesTestsListOfSales
    {
        [Fact]
        public void CheckCorrectListOfSales()
        {
            var optionsBuilder = new DbContextOptionsBuilder<ErpSystemDbContext>().UseInMemoryDatabase("testListOfSales");
            ...
            var result = sale.ListOfSales(null, null).ToList();

            Assert.True(result.Count() == 5);
            Assert.True(result[0].TotalSalePrice == 144);
            Assert.True(result[1].TotalSalePrice == 576);
            Assert.Equal(834.85m, result[2].TotalSalePrice, 2);
            Assert.Equal(1578.42m, result[3].TotalSalePrice, 2);
            Assert.Equal(15.18m, result[4].TotalSalePrice, 2);
            Assert.True(result.FirstOrDefault().SaleDate.Date == DateTime.UtcNow.Date);
            Assert.Equal(271m, result.Sum(x => x.NumberOfSoldProducts), 0);

            var resultOneProduct = sale.ListOfSales(null, "Product 1001").ToList();

            Assert.True(resultOneProduct.Count() == 2);
            Assert.True(resultOneProduct[0].TotalSalePrice == 144);
            Assert.True(resultOneProduct[1].TotalSalePrice == 576);
            Assert.True(resultOneProduct.FirstOrDefault().SaleDate.Date == DateTime.UtcNow.Date);
            Assert.Equal(10m, resultOneProduct.Sum(x => x.NumberOfSoldProducts), 0);

            var resultOneCustomer = sale.ListOfSales("Company1", null).ToList();

            Assert.True(resultOneCustomer.Count() == 5);
            Assert.True(resultOneCustomer[0].TotalSalePrice == 144);
            Assert.True(resultOneCustomer[1].TotalSalePrice == 576);
            Assert.Equal(834.85m, resultOneCustomer[2].TotalSalePrice, 2);
            Assert.Equal(1578.42m, resultOneCustomer[3].TotalSalePrice, 2);
            Assert.Equal(15.18m, resultOneCustomer[4].TotalSalePrice, 2);
            Assert.True(resultOneCustomer.FirstOrDefault().SaleDate.Date == DateTime.UtcNow.Date);
            Assert.Equal(271m, resultOneCustomer.Sum(x => x.NumberOfSoldProducts), 0);
        }
    }
}
