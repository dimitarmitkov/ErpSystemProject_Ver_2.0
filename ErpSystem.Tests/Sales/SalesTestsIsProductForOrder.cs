namespace ErpSystem.Tests.Sales
{
    ...
    public class SalesTestsIsProductForOrder
    {
        [Fact]
        public void CheckCorrectIsProductForOrder()
        {
            var optionsBuilder = new DbContextOptionsBuilder<ErpSystemDbContext>().UseInMemoryDatabase("testIsProductForOrder");
            ...
            sale.IsProductForOrder(firstProduct.Id);

            var result = dbContext.DeliveryNeededProducts.Select(x => x).ToList();
            ...
            result = dbContext.DeliveryNeededProducts.Select(x => x).ToList();

            Assert.True(result.Count() == 1);
            Assert.True(result[0].Product == "Product 1001");
            Assert.True(result[0].ProductsAvailable == 4);
            Assert.True(result[0].TotalDeliveryTime == 20);
            ...
            Assert.True(result[1].ProductsAvailable == 96);
            Assert.True(result[1].TotalDeliveryTime == 32);
            ...
            Assert.True(result[2].ProductsAvailable == 229);
            Assert.True(result[2].TotalDeliveryTime == 110);
        }
    }
}
